...
#!/bin/bash
  ...
  select opt in $OPTIONS; do
	...
	select opt2 in $OPTIONS2; do
      ...
      # Run the single imaging for normal brightness
	    sudo scanimage -d $device --resolution 1200 --format tiff --mode color --source 'TPU8x10' --focus-position 'Focus 2.5mm above glass' > $filename
            echo "Scanning"
            echo "Scan complete!"
    ...
    # Run the single imaging for darker images
            ...
            sudo scanimage -d $device --resolution 1200 --format tiff --mode color --source 'TPU8x10' --focus-position 'Focus 2.5mm above glass' --brightness -3 > $filename
            echo "Scanning"
            echo "Scan complete!"
            echo "Image saved as $filename"
            echo "<Press any key to continue>"
  ...
  sudo scanimage -d $device --resolution 1200 --format tiff --mode color --source 'TPU8x10' --focus-position 'Focus 2.5mm above glass' > $filenametime
